...
- ~~[ ] Optional "Quick" mode, which skips already-seen events faster than renpy text skip~~ Decided to cut this, would be very tedious to implement 'short' versions of events for this mode
...
- [x] Event viewer menu, which allows the player to revisit any scene that they have already seen. **Note: functional, but has following limitations**.
  - Cannot be launched from main menu (this may be fixed)
  - While variables from current playthrough are cached to avoid changing values partway through a run, anything that depends on current values will be read from current values. For example, branching paths that depend on player stats or anything that calls the relationship value screen will reflect the current playthrough.
